...
・reactはボタンに対してクリックイベントを割り当てるが、reactの場合はイベントなどがキャメルケースになる。
ラクダみたいに単語の繋ぎ部分が大文字になる
...
messageと書かずに文字列を渡せるので必須の知識といったところか
